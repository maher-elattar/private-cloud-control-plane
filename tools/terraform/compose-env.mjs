...
const settings = {
  ...
  // The pool the scoped token is granted `VM.Allocate` on. Creating into it is what stops the
  // deployment losing a usable VMID every time an instance is destroyed.
  PROXMOX_POOL_ID: 'control-plane-lab',
...
/**
 * Settings already in the file that this tool does not own, preserved verbatim.
 *
 * WHY: `tools/console/demo-user.mjs` writes `LOCAL_OIDC_USERS` into the same file, because both
 * are inputs to the same Compose stack and both must stay out of git. Rewriting the file wholesale
 * silently deleted that line, and the symptom was a stack that came up healthy with a login nobody
 * could pass — the console reported the identity provider as unavailable rather than the
 * credential as missing, because from its side those look identical.
 *
 * Anything this tool derives always wins; only unrecognised keys survive.
 */
const preserved = Object.fromEntries(
  (await readFile(OUTPUT_PATH, 'utf8').catch(() => ''))
    .split('\n')
    .map((line) => line.trim())
...
${Object.entries({ ...settings, ...preserved })
