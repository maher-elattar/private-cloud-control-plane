 ...
 */
import { ProviderTaskState, ValidationCheckState } from '@private-cloud/contracts/provider';
...
/** A run reader that answers with one scripted run. */
function runsDouble(run: Awaited<ReturnType<TerraformRunReader['readRun']>>): TerraformRunReader {
  return { readRun: async () => run };
}

function provider(
  runner = runnerDouble(),
  runs: TerraformRunReader = runsDouble(null),
): TerraformProxmoxProvider {
