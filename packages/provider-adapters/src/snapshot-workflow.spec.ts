...
import type { SnapshotCreateRequestedV1 } from '@private-cloud/contracts';
...
const SNAPSHOT_ID = '00000000-0000-4000-8000-0000000000d1';
...
function command(): SnapshotCreateRequestedV1 {
  return {
...
class MemoryWorkflowStore implements WorkflowStore {
  ...
  public constructor(private readonly power: SnapshotCreateRequestedV1) {}
  ...
  public claimNext(): Promise<ClaimedWorkflow | null> {
    ...
    return Promise.resolve({
      action: 'create_snapshot' as const,
      command: this.power,
...
describe('SnapshotWorkflow', () => {
  /** Builds a workflow for one of the three snapshot actions against a seeded VM. */
  function workflowFor(action: 'create_snapshot' | 'rollback_snapshot' | 'delete_snapshot') {
  ...
  it('declares only the submit stage as a mutation, for every action', () => {
    // Rollback discards everything written since the snapshot and delete destroys the only copy
    // of that state. Neither may be retried on an ambiguous outcome.
    for (const action of ['create_snapshot', 'rollback_snapshot', 'delete_snapshot'] as const) {
      const workflow = workflowFor(action);
      const stages = (workflow as unknown as { mutationStages: readonly WorkflowStage[] })
        .mutationStages;
      expect([...stages]).toEqual(['submitting_snapshot']);
      expect(workflow.action).toBe(action);
    }
  });

  it('creates a snapshot and confirms it appears in the provider listing', async () => {
    const store = new MemoryWorkflowStore(command());
    const provider = new FakeProvider();
    store.providerResourceId = await seedOwnedInstance(provider);
    const workflow = new SnapshotWorkflow('create_snapshot', store, provider, 'worker-test');
  ...
  it('refuses a workflow whose action is not its own', async () => {
    // The registry routes on action; reaching the wrong executor is a wiring bug that must be
    // loud, because the claim has already taken the lease.
    const store = new MemoryWorkflowStore(command());
    const provider = new FakeProvider();
    store.providerResourceId = await seedOwnedInstance(provider);
    const workflow = new SnapshotWorkflow('delete_snapshot', store, provider, 'worker-test');
    await expect(workflow.runOne()).rejects.toThrowError(/action/);
  });

  it('sends an unknown create outcome to manual review rather than retrying it', async () => {
    const store = new MemoryWorkflowStore(command());
    const provider = new FakeProvider({
      script: { createSnapshot: [{ mode: 'unknown-outcome' }] },
    });
    store.providerResourceId = await seedOwnedInstance(provider);
    const workflow = new SnapshotWorkflow('create_snapshot', store, provider, 'worker-test');
    for (let turn = 0; turn < 12 && !store.terminal; turn += 1) {
